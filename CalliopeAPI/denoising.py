    ...
    for i in np.arange(3, height - 3):
        for j in np.arange(3, width - 3):
            ...
            for k in range(1, len(neighbors) - 1):
                if (abs(neighbors[k][0] - neighbors[k - 1][0]) < 10 and abs(
                        neighbors[k][1] - neighbors[k - 1][1]) < 10 and abs(
                        neighbors[k][1] - neighbors[k - 1][1]) < 10) or (
                        abs(neighbors[k][0] - neighbors[k + 1][0]) < 10 and abs(
                        neighbors[k][1] - neighbors[k + 1][1]) < 10 and abs(
                        neighbors[k][1] - neighbors[k + 1][1]) < 10):
                    averaging.append(neighbors[k])

            r, g, b = 0, 0, 0
            length = len(averaging)
